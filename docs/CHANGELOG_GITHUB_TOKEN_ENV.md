...
1. **`src/gerrit_reporting_tool/cli.py`**
...
```yaml
- name: Generate Report
  run: gerrit-reporting-tool generate --project ONAP --repos-path ./repos
  env:
    GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
```
